import { DecorationType, ColumnType, RowContentType, BlockType, RowType } from "./types";

export const idToUuid = (path: string) =>
  `${path.slice(0, 8)}-${path.slice(8, 4)}-${path.slice(12, 4)}-${path.slice(16, 4)}-${path.slice(
    20
  )}`;
...
export const getNotionValue = (
  ...
  switch (type) {
    ...
    case "person":
      return val.filter((v) => v.length > 1).map((v) => v[1]![0][1] as string) || [];
    case "checkbox":
      return val[0][0] === "Yes";
    case "date":
    ...
    case "file":
      return val
        ...
        .map((v) => {
          ...
          const url = new URL(
            `https://www.notion.so${
              rawUrl.startsWith("/image") ? rawUrl : `/image/${encodeURIComponent(rawUrl)}`
            }`
          );
