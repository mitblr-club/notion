...
import { RowContentType, CollectionType, RowType, HandlerRequest } from "../api/types";
...
export const getTableData = async (
  ...
  const table = await fetchTableData(collection.value.id, collectionViewId, notionToken);
...
export async function tableRoute(req: HandlerRequest) {
  ...
  } = Object.keys(page.recordMap.collection_view).map((k) => page.recordMap.collection_view[k])[0];

  const { rows } = await getTableData(collection, collectionView.value.id, req.notionToken);
